...
class FormMatrix:
    def __init__(self, grammar):
        ...
        self.print_matrix(self.equal_matrix, "equal")

        self.first_matrix = self.cal_matrix("firstvt")
        self.print_matrix(self.first_matrix, "firstvt", columns=self.ts, index=self.non_ts)
        self.last_matrix = self.cal_matrix("lastvt")
        self.print_matrix(self.last_matrix, "lastvt", columns=self.ts, index=self.non_ts)

        self.priority_matrix = self.construct_priority_matrix(self.first_matrix, self.last_matrix, self.equal_matrix)
