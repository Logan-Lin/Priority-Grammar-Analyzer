...
def get_first(non_t, first_index=0):
    """
    ...
    for formula in get_all_formulas(non_t):
        ...
        if first_sym.isupper():
            if 'e' in list(get_all_formulas(first_sym)):
                # If the first symbol can be inferred to empty.
                first |= get_first(non_t, first_index=first_index + 1)
                formula_first |= get_first(non_t, first_index=first_index + 1)
...
def append_first(non_t):
    ...
    for formula in get_all_formulas(non_t):
        ...
        if first_sym.isupper() and first_sym == non_t:
            first_dict[(non_t, formula)] |= get_first(first_sym)
            ...
            if non_t in formula_list:
                ...
                if index == len(formula_list) - 1:
                    if not non_t == non_terminal:
                        ...
                        try:
                            follow_dict[non_t] |= follow_dict[non_terminal]
                        except KeyError:
                            continue
...
if __name__ == "__main__":
    init_grammar("OperatorPriority/data/Grammar3.csv")
    print_grammar()
    construct_first()
    construct_follow(start_symbol="E")
